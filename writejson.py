...
def WriteStreamNetwork(network,para):
    ...
    if (network == "tcp" and para=="none"):
        streamfile=file("/usr/local/V2ray-One-Key/json_template/tcp.json")
        tcp=json.load(streamfile)
        config[u"inbound"][u"streamSettings"]=tcp
        Write()
    if (network == "tcp" and para != "none"):
        streamfile=file("/usr/local/V2ray-One-Key/json_template/http.json")
